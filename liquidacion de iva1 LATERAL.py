...
class FormularioContable:
    def __init__(self, root):
        ...
        self.root.geometry("1000x1100")  # Aumenté el ancho para dar espacio al nuevo campo
        ...
        # Área de resultados
        self.resultado_texto = tk.Text(root, height=15, width=110)
    ...
    def crear_secciones(self):
        ...
        tk.Label(frame_cliente, text="Logo de la Empresa:").grid(row=5, column=0, sticky="w")
        self.logo_path = tk.Entry(frame_cliente, width=40, state="readonly")
        self.logo_path.grid(row=5, column=1)
        tk.Button(frame_cliente, text="Seleccionar Logo", command=self.seleccionar_logo).grid(row=5, column=2, padx=5)
    ...
    def seleccionar_logo(self):
        ...
        if file_path:
            self.logo_path.config(state="normal")
            self.logo_path.delete(0, tk.END)
            self.logo_path.insert(0, file_path)
            self.logo_path.config(state="readonly")
    ...
    def calcular_monto_bruto(self, event):
        try:
            ...
            self.calcular_subtotal_otros(None)
            self.calcular_subtotal_ret_nacionales(None)
        except ValueError:
            self.monto_bruto.config(state="normal")
            self.monto_bruto.delete(0, tk.END)
    ...
    def limpiar_formulario(self):
        for entry in [self.fecha_comprobante, self.num_comprobante_part1, self.num_comprobante_part2,
                      ...
                      self.retencion_iva, self.retencion_ganancias, self.retencion_sicoss, self.logo_path]:
            entry.delete(0, tk.END)
        for _, monto in self.retenciones_iibb:
            monto.delete(0, tk.END)
        for entry in [self.iva, self.subtotal_otros, self.subtotal_ret_nacionales, self.monto_bruto, self.logo_path]:
            entry.config(state="normal")
            entry.delete(0, tk.END)
            if entry != self.logo_path:
                entry.insert(0, "0.00")
    ...
    def consultar_registros(self):
        ...
        for i, reg in enumerate(self.registros):
            self.resultado_texto.insert(tk.END, f"Registro {i+1}: {reg['fecha_comprobante']} | {reg['num_comprobante_part1']}-{reg['num_comprobante_part2']} | {reg['tipo_comprobante']} | {reg['fecha_contabilizacion']} | {reg['cliente']['denominacion']} | Logo: {reg['cliente']['logo_path']} | Neto: {reg['factura']['neto']:.2f} | IVA: {reg['factura']['iva']:.2f} | Monto Bruto: {reg['monto_bruto']:.2f}\n")
    ...
    def editar_registro(self):
        ...
        logo_path = tk.Entry(frame_cliente, width=40, state="readonly"); logo_path.insert(0, cliente["logo_path"]); logo_path.grid(row=5, column=1)
        ...
        tk.Label(frame_cliente, text="Logo de la Empresa:").grid(row=5, column=0)
        tk.Button(frame_cliente, text="Seleccionar Logo", command=lambda: self.seleccionar_logo_edit(logo_path)).grid(row=5, column=2, padx=5)
    ...
    def imprimir_registros(self):
        ...
        with open("registros_contables.txt", "w", encoding="utf-8") as f:
            ...
            for i, reg in enumerate(self.registros):
                ...
                f.write(f"Cliente: {reg['cliente']['denominacion']} | CUIT: {reg['cliente']['cuit']} | Logo: {reg['cliente']['logo_path']}\n")
